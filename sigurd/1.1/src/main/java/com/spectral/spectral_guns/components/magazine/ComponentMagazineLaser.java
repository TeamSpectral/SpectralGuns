...
public class ComponentMagazineLaser extends ComponentGeneric
{
	...
	public ComponentMagazineLaser(Component[] required, Component[] incapatible, ComponentMaterial material, LaserColor color, int capacity)
	{
	...
	public int setAmmo(int ammo, ItemStack stack, World world, EntityPlayer player, ArrayList<Component> components)
	{
		...
		while(ammo > 0 && compound.getInteger(AMMO)+battery() <= capacity*ammoMultiplier)
		{
			compound.setInteger(AMMO, compound.getInteger(AMMO)+battery());
			--ammo;
		}
