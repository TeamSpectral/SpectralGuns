...
public abstract class ComponentMagazine extends ComponentGeneric
{
	...
	{
		super("magazine_" + id, "magazine." + name, required, incapatible, true, Type.MAGAZINE, material);
	...
	public int setAmmo(int ammo, ItemStack stack, World world, EntityPlayer player, ArrayList<Component> components)
	{
		...
		{
			compound.setInteger(AMMO, capacity);
		}
	...
	{
		return kickback + this.kickback*0.83F;
	}
	...
	public boolean isValid(ArrayList<Component> ecs)
	{
		...
		{
			if(ecs.get(i) instanceof ComponentMagazine && ecs.get(i).type == Type.MAGAZINE)
			{
				if(ecs.get(i) instanceof ComponentMagazine)
				{
					if(((ComponentMagazine)ecs.get(i)).ammoItem() != null && ammoItem() != null && ((ComponentMagazine)ecs.get(i)).ammoItem() == ammoItem())
					{
						++count;
					}
					else
